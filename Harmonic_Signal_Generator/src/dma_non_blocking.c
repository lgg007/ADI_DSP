...
/*********************************************************************************/
/*!
* @file      dma_non_blocking.c
*
* @brief     This is the experimental Harmonic Signal Generator demo.
* 			 It is created from Harmonic Sinewave series similar to the Fourier Series summation.
* 			 This is an BF706 implementation used for non-profit and is intended as educational material only!
* 			 I have used similar logic seen in the book Embedded-Signal-Processing-Micro-Architecture
...
#define DEFAULT_BLINK_RATE  10000
...
struct LedBlinker
{
	...
	bool		FreqUp;		    // Shifts Freq Up
	bool		FreqDn;			// Shifts Freq Dn
	uint32_t	Freq;
...
extern void frequp(void);
extern void freqdn(void);
...
void WelcomeMessage(void)
{
    ...
    DEBUG(1,""_VTAB_"");
    DEBUG(1,"\n");
    DEBUG(1, "Fourier Series, Harmonic signal generator with adjustable frequency!" _NL_ "");
    ...
    DEBUG(1, "aud on          (by Default is off, Turns sine wave audio on)" _NL_ "");
    DEBUG(1, "aud off         (Turns sine wave audio off)" _NL_"");
    DEBUG(1, "freq up(+)      (Turns wave Frequency Up)" _NL_ "");
    DEBUG(1, "freq dn(-)      (Turns wave Frequency Down)" _NL_"");
    DEBUG(1, "mode up(*)      (Turns wave mode Up)" _NL_ "");
    DEBUG(1, "mode dn(/)      (Turns wave mode Down)" _NL_"");
...
int main(void)
{
    ...
    pBlinker->FreqUp = false;
    pBlinker->FreqDn = false;
	   ...
	   idxRx ++; // increment to the next storable address
		...
		if(RxBuffer[0] == '\r')
		{
		   ...
		   else if (strcmp((const char *)RxCommandBuffer, "AUD OFF") == 0)
		   {
			   ...
			   pBlinker->FilterOn = false; // turn off filter
			   disable_SPORT(); // Turn off sport0 and DMA Mode
			   bToggle = true;
			   bOnce = true;
		   }
		   else if (strcmp((const char *)RxCommandBuffer, "FREQ UP") == 0)
		   {

			   pBlinker->FreqUp = true;
			   TermAction = FREQ_UP;
		   }
		   else if (strcmp((const char *)RxCommandBuffer, "FREQ DN") == 0)
		   {

			   pBlinker->FreqDn = true;
			   TermAction = FREQ_DN;
		   }
		   else if (strcmp((const char *)RxCommandBuffer, "MODE UP") == 0)
		   {
			   modeup();
			   TermAction = MODE_UP;
		   }
		   else if (strcmp((const char *)RxCommandBuffer, "MODE DN") == 0)
		   {
			   modedn();
			   TermAction = MODE_DN;
		   }
		...
		else if(RxBuffer[0] == '+' || RxBuffer[0] == '-' || RxBuffer[0] == '/' || RxBuffer[0] == '*')
		{
			if (RxBuffer[0] == '+')
			{
				pBlinker->FreqUp = true;
		...
		switch (TermAction)
		{
			...
			case AUD_OFF:
				...
				break;
			case MODE_UP:
				DEBUG(1,"[MODE UP]" _NL_ "");
				break;
			case MODE_DN:
				DEBUG(1,"[MODE DN]" _NL_ "");
				break;
			case FREQ_UP:
				DEBUG(1,"[FREQ UP]" _NL_ "");
				break;
			case FREQ_DN:
				DEBUG(1,"[FREQ DN]" _NL_ "");
				break;
			case FILT_ON:
